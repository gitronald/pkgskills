# Plans

| # | Plan | Status | Concluded | PR |
|---|---|---|---|---|
| 004 | [Track only a skill-level version in stub metadata](plans/004-per-skill-and-host-versions/plan.md) | done | 2026-09-11 19:00 PT | [#5](https://github.com/gitronald/pkgskills/pull/5) |
| 004 | [Track only a skill-level version in stub metadata](plans/004-per-skill-and-host-versions/plan.md) | active | — | — |
| 007 | [Close the gaps a migrating host hits on adoption](plans/007-migrating-host-adoption-gaps/plan.md) | draft | — | — |
| 006 | [Sync tooling to proj-template 0.10.0](plans/006-template-upgrade-0-10-0/plan.md) | done | 2026-09-11 10:44 PT | [#2](https://github.com/gitronald/pkgskills/pull/2) |
| 005 | [Rename the package to pkgskills](plans/005-rename-to-pkgskills/plan.md) | done | 2026-09-11 10:32 PT | [#1](https://github.com/gitronald/pkgskills/pull/1) |
| 003 | [Store skill sources as spec-conformant SKILL.md directories](plans/003-spec-conformant-skill-sources/plan.md) | done | 2026-09-09 23:13 PT | — |
| 000 | [Support hosts with several single-source skills and print-only docs](plans/000-multi-skill-hosts/plan.md) | done | 2026-09-09 22:12 PT | — |
| 002 | [Backport planners install and release lessons](plans/002-planners-install-lessons/plan.md) | done | 2026-09-09 12:26 PT | — |
| 001 | [Sync repo tooling to the current project template](plans/001-template-upgrade/plan.md) | done | 2026-09-09 09:37 PT | — |

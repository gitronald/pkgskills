...
def test_foreign_shapes(box: Sandbox) -> None:
    ...
    # A stamp the host wrote itself before it adopted pkgskills: same prefix,
    # no `via pkgskills` token. Still foreign, but for its own reason.
    path.write_text("<!-- generated by solohost 1.0; do not edit -->\n")
    row = inst.check_artifact(SOLO, skill, "local", box.repo)
    assert row.status == "foreign"
    assert row.reason == (
        "stamped by an earlier solohost release, before pkgskills; --force replaces it"
    )

...
A skill with several sources becomes a dispatcher: each source's stem is a
subcommand, and the stub tells the agent to run `<cli> skill <subcommand>`.
...
`install --check` re-renders each artifact and compares, with both version
tokens masked, so upgrading either package never flags a file whose content

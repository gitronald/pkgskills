...
- A file stamped by an earlier release of the host itself, before that host
  adopted `pkgskills`, no longer reports the nonsensical `generated by <dist>,
  not <dist>`. It is still `foreign` — `--force` is still the remedy — but the
  reason now names what it actually is. A stamp from a *different*
  distribution reports the same message as before.
